...
(function (Handsontable) {
  ...
  function ContextMenu(instance, customOptions) {
    ...
    this.markSelected = function (label) {
      return "<span class='selected'>Р Р†РЎС™РІР‚Сљ</span>" + label;
    };
...
var addMergeActionsToContextMenu = function (defaultOptions) {
  ...
  defaultOptions.items.push({
    ...
    name: function () {
      ...
      if (info) {
        return 'Разъединить ячейки';
      }
      else {
        return 'Объединить ячейки';
      }
    },
    callback: function () {
		...
		if (range) {
			...
			if (this.mergeCells.canMergeRange(range) && !this.mergeCells.mergedCellInfoCollection.getInfo(range.from.row, range.from.col)){
				
				var range_coords = this.getSelected();
				if (range_coords){			
					for (var r = range_coords[0]; r <= range_coords[2]; r++){
						for (var c = range_coords[1]; c <= range_coords[3]; c++){
							if (this.mergeCells.mergedCellInfoCollection.getInfo(r, c))
								// getInfo returns undefined if cell has not any merges
								this.mergeCells.unmergeSelection(new WalkontableCellCoords(r, c));
						}
					}
				}			
			}
